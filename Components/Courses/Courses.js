...
import { Container, Card, Row, Col, Button, ListGroup, Form, Nav } from 'react-bootstrap';
...
import { getFirestore, addDoc, arrayUnion, updateDoc, collection, doc, setDoc, Timestamp, Firestore, serverTimestamp, query, where, getDocs, getDoc, orderBy, limit, DocumentSnapshot } from 'firebase/firestore'; 
...
class Courses extends React.Component {
 ...
 handleReplySubmit = async (event) => {
    ...
    renderModule = () => {
      ...
      const q = query(commentsRef, orderBy("time"), limit(80));
        ...
        return(
            ...
            <hr></hr>
              ...
              <hr></hr>
                ...
                { comments && comments.filter((bit) => bit.commentType==="original").map(cmt => {
                  ...
                  <div><strong>{cmt.displayName}</strong></div>
                  <div>{cmt.comment}</div>
                  </Card>
                  ...
                  onClick={getAuth().currentUser ? () => updateDoc(doc(db, "comments", cmt.docID), {likers: arrayUnion(getAuth().currentUser.uid)}): null}
                  >Like</Button>
                  <Button variant="secondary" style={{height: 25, fontSize: 13, marginLeft: 10, marginRight: 20, textAlign: "center", padding: 3, width: 50}}
                  onClick={() => this.setState({replyActive: cmt.docID})}>Reply</Button>
                  </>
                  : null } 
                  <div>{minutes < 1 ? "Just now" : 
                  ...
                  {getAuth().currentUser & this.state.replyActive===cmt.docID ?
                   ...
                   : getAuth().currentUser === null & this.state.replyActive===cmt.docID ?
                   <div>Sign up or login to post comments and respond to other comments.</div>
                   : null
                  ...
                  {getAuth().currentUser ? 
                  <>
                  <Button variant="secondary" style={{height: 25, fontSize: 13, textAlign: "center", padding: 3, width: 50, marginRight: 60}}
                  onClick={getAuth().currentUser ? () => updateDoc(doc(db, "comments", thingy.docID), {likers: arrayUnion(getAuth().currentUser.uid)}) : null}
                  >Like</Button>
                  </> : null }
                  <div>{minutes < 1 ? "Just now" : 
                  minutes > 0 & hours < 1 ? minutes + " m" :
